...
print(output['text'])
```
###### Note: A prompt modifier is basically a list where each item is a piece of text to be added pre (attached before) and/or post (attched after) the query/prompt. For example, the first item of pre/post values will be added to the initial query/prompt, and the second item of the pre/post values will be added to the returned LLM response, and the resulting follow-up query/prompt will then be automatically sent to the LLM. This repeats for the number of items specified in the prompt modifier list - essentially a method of "chaining" a pattern of prompts and responses to LLMs together in a looped manner.
